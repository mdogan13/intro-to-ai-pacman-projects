...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # If the noisy distance is None, the ghost is in the prison(eaten),
        # updates the believes accordingly(i.e. the ghost is in the prison and nowhere else).
        for p in self.legalPositions:
            if not noisyDistance == None :
                # Ghost is not eaten
                updated_beliefs[p] = emissionModel[util.manhattanDistance(p, pacmanPosition)] * self.beliefs[p]
            else:
                # Ghost is eaten
                updated_beliefs[p] = 0
                updated_beliefs[self.getJailPosition()] = 1

        # Update self.beliefs with the normalized new beliefs
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # Store the new beliefs in a counter
        updated_beliefs = util.Counter()

        # Calculate a new position distribution for each position
        for oldPos, dummy in self.beliefs.items():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            # Calculate the new probabilities for new distributions and store them in updated_beliefs
            for newPos, newProb in newPosDist.items():
                updated_beliefs[newPos] = updated_beliefs[newPos] + newProb * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particles = [self.legalPositions[p % len(self.legalPositions)] for p in range(numOfParticles)]
    ...
    def observe(self, observation, gameState):
        ...
        if not noisyDistance == None:
            ...
            for location in self.legalPositions:
                ...
                updated_beliefs[location] = emissionModel[trueDistance] * currentBeliefs[location]
            if updated_beliefs.totalCount() == 0:
                # All particles have 0 weight, recreate them by calling initializeUniformly()
                self.initializeUniformly(gameState)
            else:
                # Generate sample
                self.particles = [util.sample(updated_beliefs) for p in self.particles]
        else:
            # Ghost is eaten
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        currentBeliefs = util.Counter()

        for p in self.particles:
            currentBeliefs[p] = currentBeliefs[p] + 1.0 / self.numParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        numOfParticles = self.numParticles
        self.particles = [self.legalPositions[p % len(self.legalPositions)] for p in range(numOfParticles)]
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.particles:
            currentBeliefs[p] = currentBeliefs[p] + 1
